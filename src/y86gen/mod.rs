...
macro_rules! add_imm {
    ($writer: expr, $reg: expr, $imm: expr) => {
        writeln!($writer, "irmovq {}, %r11", $imm)?;
        writeln!($writer, "addq %r11, {}", $reg)?;
    };
}
...
macro_rules! store_to_stack {
    ($writer: expr, $offset: expr, $src: expr) => {
        writeln!($writer, "rmmovq {}, {}(%rsp)", $src, $offset)?;
    };
}
...
fn store(writer: &mut impl Write, dest: Value, prog: &Program, info: &GenRiscvInfo, reg: &str) -> Result<()> {
    if info.globalmap.get(&dest).is_some() {
        writeln!(writer, "irmovq {}, %r11", prog.borrow_value(dest).name().as_ref().unwrap()[1..].to_string())?;
        writeln!(writer, "rmmovq %rax, (%r11)")?;
    } else {
